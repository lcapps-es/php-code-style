...
use ECSPrefix202401\Symfony\Component\OptionsResolver\Exception\InvalidOptionsException;

final class NamedArgumentsFixer extends AbstractFixer implements ConfigurableFixerInterface
{
    /**
     * @internal
     */
     ...
     */
    private static $allowedValues = [self::ALIGN, self::ALIGN_BY_SCOPE, self::ALIGN_SINGLE_SPACE, self::ALIGN_SINGLE_SPACE_MINIMAL, self::ALIGN_SINGLE_SPACE_BY_SCOPE, self::ALIGN_SINGLE_SPACE_MINIMAL_BY_SCOPE, self::SINGLE_SPACE, self::NO_SPACE, self::AT_LEAST_SINGLE_SPACE, null];
    /**
...
', ['operators' => ['=>' => 'align_by_scope']]), new CodeSample('<?php
$array = [
    "foo" => 12,
    "baaaaaaaaaaar"  => 13,
...
', ['operators' => ['=>' => 'align_single_space_by_scope']]), new CodeSample('<?php
$array = [
    "foo" => 12,
    "baaaaaaaaaaar"  => 13,
...
', ['operators' => ['=>' => 'align_single_space_minimal_by_scope']])]);
        ...
        return new FixerConfigurationResolver([(new FixerOptionBuilder('default', 'Default fix strategy.'))->setDefault(self::SINGLE_SPACE)->setAllowedValues(self::$allowedValues)->getOption(), (new FixerOptionBuilder('operators', 'Dictionary of `binary operator` => `fix strategy` values that differ from the default strategy. Supported are: ' . Utils::naturalLanguageJoinWithBackticks(self::SUPPORTED_OPERATORS) . '.'))->setAllowedTypes(['array'])->setAllowedValues([static function (array $option) : bool {
            foreach ($option as $operator => $value) {
                if (!\in_array($operator, self::SUPPORTED_OPERATORS, \true)) {
                    throw new InvalidOptionsException(\sprintf('Unexpected "operators" key, expected any of %s, got "%s".', Utils::naturalLanguageJoin(self::SUPPORTED_OPERATORS), \gettype($operator) . '#' . $operator));
                }
                if (!\in_array($value, self::$allowedValues, \true)) {
                    throw new InvalidOptionsException(\sprintf('Unexpected value for operator "%s", expected any of %s, got "%s".', $operator, Utils::naturalLanguageJoin(\array_map(static function ($value) : string {
                        return Utils::toString($value);
                    }, self::$allowedValues)), \is_object($value) ? \get_class($value) : (null === $value ? 'null' : \gettype($value) . '#' . $value)));
                }
            }
            return \true;
    ...
    private function fixWhiteSpaceAroundOperator(Tokens $tokens, int $index) : void
    {
        ...
        if (self::ALIGN === $this->operators[$tokenContent] || self::ALIGN_BY_SCOPE === $this->operators[$tokenContent]) {
            return;
        }
        // fix white space after operator
        if ($tokens[$index + 1]->isWhitespace()) {
            if (self::ALIGN_SINGLE_SPACE_MINIMAL === $this->operators[$tokenContent] || self::ALIGN_SINGLE_SPACE_MINIMAL_BY_SCOPE === $this->operators[$tokenContent]) {
    ...
    private function fixAlignment(Tokens $tokens, array $toAlign) : void
    {
        ...
        foreach ($toAlign as $tokenContent => $alignStrategy) {
            ...
            if (self::ALIGN_SINGLE_SPACE === $alignStrategy || self::ALIGN_SINGLE_SPACE_MINIMAL === $alignStrategy || self::ALIGN_SINGLE_SPACE_BY_SCOPE === $alignStrategy || self::ALIGN_SINGLE_SPACE_MINIMAL_BY_SCOPE === $alignStrategy) {
                if ('=>' === $tokenContent) {
                    for ($index = $tokens->count() - 2; $index > 0; --$index) {
    ...
    private function injectAlignmentPlaceholdersDefault(Tokens $tokens, int $startAt, int $endAt, string $tokenContent) : void
    {
        ...
        for ($index = $startAt; $index < $endAt; ++$index) {
            ...
            if (\strtolower($content) === $tokenContent && $this->tokensAnalyzer->isBinaryOperator($index) && ('=' !== $content || !$this->isEqualPartOfDeclareStatement($tokens, $index)) && $newLineFoundSinceLastPlaceholder) {
    ...
    private function injectAlignmentPlaceholdersForArrow(Tokens $tokens, int $startAt, int $endAt) : void
    {
        ...
        for ($index = $startAt; $index < $endAt; ++$index) {
            ...
            if ($token->isGivenKind(\T_YIELD)) {
                $yieldFoundSinceLastPlaceholder = \true;
            }
            if ($token->isGivenKind(\T_FN)) {
                $yieldFoundSinceLastPlaceholder = \false;
                $from = $tokens->getNextMeaningfulToken($index);
    ...
    private function replacePlaceholders(Tokens $tokens, string $alignStrategy, string $tokenContent) : string
    {
        ...
        for ($j = 0; $j <= $this->deepestLevel; ++$j) {
            ...
            foreach ($lines as $index => $line) {
                ...
                } elseif (self::ALIGN_BY_SCOPE !== $alignStrategy && self::ALIGN_SINGLE_SPACE_BY_SCOPE !== $alignStrategy && self::ALIGN_SINGLE_SPACE_MINIMAL_BY_SCOPE !== $alignStrategy) {
                    ++$groupIndex;
                    $groups[$groupIndex] = [];
                }
            }
            foreach ($groups as $group) {
                ...
                if (self::ALIGN !== $alignStrategy) {
                    ...
                    foreach ($group as $index) {
                        ...
                        } elseif (self::ALIGN_SINGLE_SPACE_MINIMAL === $alignStrategy || self::ALIGN_SINGLE_SPACE_MINIMAL_BY_SCOPE === $alignStrategy) {
                            if (!Preg::match('/^\\h+$/', $before)) {
